...
class BlockchainController {
    ...
    constructor(app: Express, blockchainObj: IBlockchain) {
        this.app = app;
    ...
    validateChain() {
        this.app.get("/validateChain", async (req, res) => {
            try {
                let errorLog: Array<IBlock> = await this.blockchain.validateChain();
                if(errorLog) {
                    return res.status(200).json(errorLog);
                } else {
                    return res.status(404).send("Failed to validate chain!");
                }
            } catch (error) {
                return res.status(500).send("An error happened!");
            }
...
module.exports = (app: Express, blockchainObj: IBlockchain) => { return new BlockchainController(app, blockchainObj);}